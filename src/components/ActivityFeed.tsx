...
export function ActivityFeed({
  ...
  return (
    <ul className="space-y-3">
      {items.map((item) => {
        const content = (
          <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-xs transition hover:border-slate-300">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {item.state_label ? (
                    <span className="text-sm text-slate-500">{item.state_label}</span>
                  ) : null}
                </div>
